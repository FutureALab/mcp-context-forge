...
class TestMemberKeyWorkflow(unittest.TestCase):
    ...
    def test_full_workflow(self):
        ...
        with httpx.Client(base_url=origin, trust_env=False, timeout=30) as public, httpx.Client(base_url=origin, trust_env=False, timeout=30) as admin:
            ...
            workbook.close()
            history_path = f"/admin/member-usage/key-history/{server}"
            history = admin.get(history_path, params={"limit": 1}).json()
            self.assertGreaterEqual(history["total"], 2)
            self.assertEqual(len(history["results"]), 1)
            self.assertNotIn("api_key", history["results"][0])
            detail = admin.post(f"{history_path}/{issued.json()['token_id']}")
            self.assertEqual(detail.status_code, 200)
            self.assertEqual(detail.json()["api_key"], key)
            history_export = admin.post(history_path + "/export")
            self.assertEqual(history_export.status_code, 200)
            self.assertIn("no-store", history_export.headers["cache-control"])
            workbook = openpyxl.load_workbook(io.BytesIO(history_export.content))
            rows = list(workbook.active.values)
            self.assertEqual(len(rows) - 1, history["total"])
            self.assertIn(key, [row[rows[0].index("API Key")] for row in rows[1:]])
            self.assertIn(email, [row[rows[0].index("成员邮箱")] for row in rows[1:]])
            workbook.close()
            for path in (history_path, history_path + "/export", f"{history_path}/{issued.json()['token_id']}"):
                denied = public.request("GET" if path == history_path else "POST", path, headers={"Authorization": f"Bearer {key}"})
